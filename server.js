...
const { 
    ...
    shouldBuyLR, 
    shouldSellLR 
} = require('./golden_strategies'); // Adjust path as necessary
...
app.post('/save-user', (req, res) => {
    ...
    const sql = `INSERT INTO users (client_code, api_key, password, totp, unique_code) VALUES (?, ?, ?, ?, ?)`;
    const values = [clientCode, apiKey, password, totp, uniqueCode];
    
    db.query(sql, values, (err, results) => {
        if (err) {
            return res.status(500).json({ success: false, message: 'Database error', error: err.message });
        }
        res.json({ success: true, message: 'User data saved successfully', userId: results.insertId, uniqueCode });
    });
});
...
app.get('/get-user/:uniqueCode', (req, res) => {
    ...
    db.query(`SELECT * FROM users WHERE unique_code = ?`, [uniqueCode], (err, rows) => {
        if (err) {
            return res.status(500).json({ success: false, message: 'Database error', error: err.message });
        }
        if (rows.length === 0) {
...
app.post('/login/:uniqueCode', async (req, res) => {
    ...
    db.query(`SELECT * FROM users WHERE unique_code = ?`, [uniqueCode], async (err, rows) => {
        if (err) {
            return res.status(500).json({ success: false, message: 'Database error', error: err.message });
        }
        if (rows.length === 0) {
        ...
        const { client_code, api_key, password, totp } = rows[0];
